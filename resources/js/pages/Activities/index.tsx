...
import { Activity, ActivityStatus, Paginator } from '@/types';
...
export default function Index({ activities: initialActivities, activityStatuses }: IndexProps) {
  //console.log('activities prop:', initialActivities);
  ...
  const handleSave = () => {
    if (editingActivity) {
      ...
      setActivities({
        ...activities,
        data: [...activities.data, newActivity],
      });
    }
    setModalOpen(false);
  };
  ...
  const handleDelete = (id: number) => {
    setActivities({
      ...activities,
      data: activities.data.filter(a => a.id !== id),
    });
  };
